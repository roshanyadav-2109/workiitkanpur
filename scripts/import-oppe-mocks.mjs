// Load the original OPPE mock papers in scripts/oppe-mock-sets.json as Test
// Series (mock) sets. Original questions, kept inside the OPPE 1 concept scope
// (weeks 1-5). Incremental: each set is removed and rebuilt by slug, so the
// other papers keep their rows and attempts.
//
//   SUPABASE_ACCESS_TOKEN=... node scripts/import-oppe-mocks.mjs           # dry run
//   SUPABASE_ACCESS_TOKEN=... node scripts/import-oppe-mocks.mjs --commit  # apply
...
console.log(`${sets.length} mock sets, ${total} questions, ${total * 5} tests`);
...
for (const set of sets) {
  ...
  const [row] = await rest("test_sets", "POST", [{
    ...
    exam: "OPPE 1",
    year: null,
    source: "coaching mock (original questions, OPPE 1 scope)",
    category: "mock",
  ...
  const created = await rest("questions", "POST", set.questions.map((q) => ({
    ...
    tags: ["oppe-1", "mock", "python", set.slug],
