// Load the PPA (Practice Programming Assignment) questions into the practice bank.
//
//   SUPABASE_ACCESS_TOKEN=... node scripts/import-ppa.mjs           # dry run
//   SUPABASE_ACCESS_TOKEN=... node scripts/import-ppa.mjs --commit  # apply
//
// Also needs NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.
//
// Re-runnable: drops previously loaded PPA rows (tagged 'ppa') and rebuilds.
...
const rows = JSON.parse(readFileSync(new URL("./ppa-week1-6-questions.json", import.meta.url), "utf8"));
...
await sql(`
  delete from public.questions
   where subject_id = '${subject.id}' and 'ppa' = any(tags);
`);
...
const after = await sql(`
  select count(*) filter (where practice_only) as practice,
         count(*) filter (where practice_only and 'ppa' = any(tags)) as grpa,
