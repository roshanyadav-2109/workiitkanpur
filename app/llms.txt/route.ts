...
export async function GET() {
  ...
  for (const s of subjects) {
    const state = s.is_active ? "practice available" : "coming soon";
    L.push(
      `- [${s.name}](${SITE_URL}/app/subjects/${s.slug}): OPPE syllabus, ` +
        `PYQs, mock tests and study guides for ${s.name} (${state}).`,
    );
  }
  L.push("");
  ...
  L.push("## Key pages");
  L.push(`- [Home](${SITE_URL}/): what OPPE Practice is and how it works.`);
  L.push(
  ...
  L.push("## About");
  L.push(
    "OPPE Practice is an independent, free study resource for students of the " +
